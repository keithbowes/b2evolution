...
class idna_convert
{
    ...
    function __construct($options = false)
    {
        ...
        if (function_exists('file_get_contents')) {
            $this->NP = unserialize(file_get_contents(dirname(__FILE__).'/_idna_convert_npdata.ser.inc'));
        } else {
            $this->NP = unserialize(join('', file(dirname(__FILE__).'/_idna_convert_npdata.ser.inc')));
        }
