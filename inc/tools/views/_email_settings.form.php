...
$Form->begin_fieldset( TB_( 'Email envelope' ).get_manual_link( 'email-notification-settings' ) );
	...
	{ // Show infomration and action buttons only for users with edit users permission
		...
		$remove_customization = ' - <a href="%s" class="ActionButton" style="float:none">'.TB_('remove customizations').'</a>';
		...
		{ // There are users with custom sender email settings
			...
			$notification_sender_email_note = get_icon( 'warning_yellow' ).' '.sprintf( TB_('<a href="%s">%d users</a> have different custom address'), url_add_param( $users_url, 'custom_sender_email=1', '&' ), $custom_sender_email_count ).$sender_email_remove_customization;
		}
		...
		{ // There are users with custom sender name settings
			...
			$notification_sender_name_note = get_icon( 'warning_yellow' ).' '.sprintf( TB_('<a href="%s">%d users</a> have different custom name'), url_add_param( $users_url, 'custom_sender_name=1', '&' ), $custom_sender_name_count ).$sender_name_remove_customization;
		}
	}

	// Display settings input fields
	$Form->email_input( 'notification_sender_email', $Settings->get( 'notification_sender_email' ), 50, TB_( 'Sender email address' ), array( 'maxlength' => 127, 'required' => true, 'note' => $notification_sender_email_note ) );
	$Form->text_input( 'notification_sender_name', $Settings->get( 'notification_sender_name' ), 50, TB_( 'Sender name' ), $notification_sender_name_note, array( 'maxlength' => 127, 'required' => true ) );
	$Form->text_input( 'notification_return_path', $Settings->get( 'notification_return_path' ), 50, TB_( 'Return path' ), '', array( 'maxlength' => 127, 'required' => true ) );
	$Form->text_input( 'notification_short_name', $Settings->get( 'notification_short_name' ), 50, TB_( 'Short site name' ), TB_('Shared with site settings'), array( 'maxlength' => 127, 'required' => true ) );
	$Form->text_input( 'notification_long_name', $Settings->get( 'notification_long_name' ), 50, TB_( 'Long site name' ), TB_('Shared with site settings'), array( 'maxlength' => 255 ) );
	$fileselect_params = array( 'file_type' => 'image', 'max_file_num' => 1, 'window_title' => TB_('Select site logo'), 'root' => 'shared_0', 'size_name' => 'fit-320x320' );
