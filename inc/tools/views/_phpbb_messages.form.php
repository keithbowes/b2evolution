...
$Form->begin_form( 'fform', $phpbb_tool_title.' - '.TB_('Step 7: Import messages') );
evo_flush();
...
$Form->begin_fieldset( TB_('Report of the mesagges import') );

	$Form->info( TB_('Count of the imported messages'), '<b>'.(int)phpbb_get_var( 'messages_count_imported' ).'</b>' );

	$Form->info( TB_('Count of the messages that are NOT imported because of missing users'), '<b class="red">'.(int)phpbb_get_var( 'messages_count_missing_users' ).'</b>' );

	$Form->info( TB_('Count of the imported replies'), (int)phpbb_get_var( 'replies_count_imported' ) );
...
$Form->buttons( array( array( 'submit', 'submit', TB_('Go to Forum'), 'SaveButton' ) ) );
