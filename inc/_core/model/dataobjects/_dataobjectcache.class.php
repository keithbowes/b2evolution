...
class DataObjectCache
{
	...
	{
		$SQL = $this->get_SQL_object( 'Get the '.$this->objtype.' rows to load the objects into the cache by '.get_called_class().'->'.__FUNCTION__.'()' );
		$SQL->WHERE($sql_where);
		return $this->load_by_sql($SQL);
	}
	...
	function load_by_sql( $SQL )
	{
		...
		{	// Set SQL title for debug info:
			$SQL->title = 'Get the '.$this->objtype.' rows to load the objects into the cache by '.get_called_class().'->'.__FUNCTION__.'()';
		}
