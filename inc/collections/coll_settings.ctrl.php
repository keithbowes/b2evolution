...
if( $action == 'dashboard' )
{
	...
	if ( $blog )
	{
		...
		echo '<div class="row">';
			...
			echo '</div>';
			echo '<h2 class="col-xs-12 col-sm-10 col-sm-pull-2 page-title">'.get_coll_fav_icon( $Blog->ID, array( 'class' => 'coll-fav' ) ).'&#160;'.$Blog->dget( 'name' ).'</h2>';
		echo '</div>';
		...
		{	// We have recent edits
			...
			{	// We have permission to add a post with at least one status:
				$block_item_Widget->global_icon( T_('Write a new post...'), 'new', '?ctrl=items&amp;action=new&amp;blog='.$Blog->ID, T_('New post').' →', 3, 4, array( 'class' => 'action_icon btn-primary' ) );
			}
			echo '<div class="items_container evo_content_block">';
