...
class Item extends ItemLight
{
	 ...
	 */
	function check_and_clear_inline_images()
	{
		global $Messages;
		...
		{ // Remove all unused inline images from the content
			$this->content = str_replace( $unused_inline_images, '', $this->content );
	...
	function get_more_link( $params = array() )
	{
		...
		{	// We're NOT in "more" mode:
			...
			{ // TRANS: this is the default text for the extended post "more" link
				$params['link_text'] = T_('Full story').' →';
	...
	function get_attached_image_tag( $Link, $params )
	{
		...
		{ // We're linking to the original image, let lighbox (or clone) quick in:
			$link_title = empty( $params['image_link_title'] ) ? '#title#' : $params['image_link_title'];	// This title will be used by lightbox (colorbox for instance)
	...
	function tags( $params = array() )
	{
		...
		if( !empty( $tags ) )
		{
			...
			foreach( $tags as $tag )
			{
				...
				if( $links )
				{	// We want links
					echo $params['tag_before'] . $this->Blog->get_tag_link( $tag ) . $params['tag_after'];
				}
				else
				{
					echo $params['tag_before'] . evo_htmlspecialchars( $tag, NULL, $evo_charset ) . $params['tag_after'];
				}
			}
			...
			foreach( $images[0] as $i => $current_link_tag )
			{
				...
				if( ! empty( $images[2][$i] ) )
				{
					...
					{ // A class name is set for the image
						$image_extraclass = strip_tags( str_replace( '.', '', $inline_params[1] ) );
						if( preg_match('#^[A-Za-z0-9]+$#', $image_extraclass ) )
