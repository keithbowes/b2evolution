...
function convert_lang_to_locale( $table, $columnlang, $columnID )
{
	...
	echo get_install_format_text_and_log( 'Converting langs to locales for '. $table. '...<br />', 'br' );
	...
	{ // converting the languages we've found
		$converted = false;
		echo get_install_format_text_and_log( '&#160; Converting lang \''. $lkey. '\' ' ); // (with IDs: '. implode( ', ', $lIDs ). ').. ';
		...
		{ // we have an old two letter lang code to convert
			...
			{  // loop given locales
				...
				{  // if language matches, update
					...
					echo get_install_format_text_and_log( 'to locale \''. $newlkey. '\'<br />', 'br' );
					break;
				}
			}
		}
		...
		{ // we have nothing converted yet, setting default:
			$DB->query( "UPDATE $table
										...
										WHERE $columnlang = '$lkey'" );
			echo get_install_format_text_and_log( 'forced to default locale \''. $default_locale. '\'<br />', 'br' );
		}
	}
	echo "\n";
...
function upgrade_b2evo_tables( $upgrade_action = 'evoupgrade' )
{
	...
	echo get_install_format_text_and_log( '<p><span class="text-muted">'.T_('Checking DB schema version...').' ', 'p-start' );
	$old_db_version = get_db_version();

	if( empty($old_db_version) )
	{
		echo get_install_format_text_and_log( '<p><strong>OOPS! b2evolution doesn\'t seem to be installed yet.</strong></p>', 'p' );
		return false;
	}
	...
	echo get_install_format_text_and_log( "OK.</span><br />\n", 'br' );
...
// fp> TMP NOTE: This was current status of antispam
	...
	if( upg_task_start( 12460, 'Upgrading email newsletter subscriptions table...' ) )
	{	// part of 6.10.0-beta
	...
	if( upg_task_start( 12470, 'Upgrading email campaigns table...' ) )
	{	// part of 6.10.0-beta
	...
	if( upg_task_start( 12735, 'Creating default item type "Content Block"...' ) )
	{	// part of 6.10.1-stable
	...
	{	// part of 7.0.0-alpha
		...
		foreach( $free_html_widgets as $free_html_widget_ID => $free_html_widget_params )
		{
			...
			$DB->query( 'UPDATE T_widget__widget
				  SET wi_params = '.$DB->quote( serialize( $free_html_widget_params ) ).'
				WHERE wi_ID = '.$free_html_widget_ID );
		}
		upg_task_end();
	}
	...
	if( upg_task_start( 15520, 'Creating table for aggregations of goal hits...' ) )
	{	// part of 7.0.2-beta
	...
	if( upg_task_start( 15580, 'Creating default item type "Homepage Content Tab"...' ) )
	{	// part of 7.0.2-beta
	...
	if( upg_task_start( 15600, 'Creating table for Menus...' ) )
	{	// part of 7.0.2-beta
	...
	if( upg_task_start( 15610, 'Creating table for Menu entries...' ) )
	{	// part of 7.0.2-beta
			...
			else
			{
				...
				if( file_exists( $plugin_dir ) && is_dir( $plugin_dir ) )
				{
					...
					else
					{
						...
						echo get_install_format_text_and_log( sprintf( '<span class="text-danger"><evo:error>Unable to delete directory: %s</evo:error></span>', $plugin_dir )."<br />\n", 'br' );
					}
				}
				if( file_exists( $plugin_filename ) && is_file( $plugin_filename ) )
				{
					...
					{
						$plugin_deleted = false;
						echo get_install_format_text_and_log( sprintf( '<span class="text-danger"><evo:error>Unable to delete file: %s</evo:error></span>', $plugin_filename )."<br />\n", 'br' );
					}
				}
				if( $plugin_deleted )
				{
				...
				{	// Obsolete skin found in the skins folder:
					...
					{	// Iterate through all available versions:
						if( ! rmdir_r( $skins_path.$folder ) )
						{
							...
							echo get_install_format_text_and_log( sprintf( '<span class="text-danger"><evo:error>Unable to delete folder %s.</evo:error></span>', $skins_path.$folder ) );
						}
					}
				}
	...
	if( upg_task_start( 16160, 'Dummy upgrade block, just to force execution of the upgrade procedure to update templates for cross-posted items...' ) )
	{	// part of 7.2.0-beta
		upg_task_end();
	}
	...
	{	// We haven't executed these general tasks yet:
		...
		echo get_install_format_text_and_log( '<h2>'.T_('Clearing caches...').'</h2>', 'h2' );
		evo_flush();
		...
		{ // Display errors of the cache checking
			foreach( $check_cache_messages as $check_cache_message )
			{
				...
				echo get_install_format_text_and_log( '<span class="text-danger"><evo:error>'.$check_cache_message.'</evo:error></span>' );
			}
			echo get_install_format_text_and_log( '<br />', 'br' );
		}
		task_end();
		...
		echo get_install_format_text_and_log( '<h2>'.T_('Letting plugins upgrade themselves...').'</h2>', 'h2' );
		evo_flush();
		...
		echo get_install_format_text_and_log( '<h2>'.T_('Checking defaults...').'</h2>', 'h2' );
		evo_flush();
		...
		{ // Max exe time not disabled and we're recahing the end
			...
			echo get_install_format_text_and_log( 'We are reaching the time limit for this script. Please click <a href="'.$upgrade_continue_url.'">continue</a>...' );
			// Dirty temporary solution:
			exit(0);
		}
		task_end();
	...
	echo get_install_format_text_and_log( "Starting to check DB...<br/>\n", 'br' );
	evo_flush();
	...
	{	// For each table in the schema, check diffs...
		...
		{
			echo get_install_format_text_and_log( '<br />', 'br' );
			echo get_install_format_text_and_log( 'Checking table: '.$table.': ' );
		}
	...
	{	// Upgrades are needed:
		...
		if( ! $confirmed_db_upgrade )
		{
			...
			echo get_install_format_text_and_log( '<p>'.T_('The version number is correct, but we have detected changes in the database schema. This can happen if you\'ve been using development versions directly off GitHub...').'</p>', 'p' );

			echo get_install_format_text_and_log( '<p>'.T_('The following database changes will be carried out. If you are not sure what this means, it will probably be alright.').'</p>', 'p' );
