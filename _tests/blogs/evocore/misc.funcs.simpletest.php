...
class MiscFuncsTestCase extends EvoUnitTestCase
{
	...
	function test_strmaxlen()
	{
		...
		$this->assertEqual( strmaxlen('1&amp;2', 5, NULL, 'htmlbody'), '1â€¦' );
		$this->assertEqual( strmaxlen('1&amp;22', 7, NULL, 'htmlbody'), '1&amp;â€¦' );
		$this->assertEqual( strmaxlen('1&amp;2', 3, NULL, 'formvalue'), '1â€¦' );
		$this->assertEqual( strmaxlen('1&    2', 3, NULL, 'formvalue'), '1&amp;â€¦' );
		$this->assertEqual( strmaxlen('1&2', 3, NULL, 'formvalue'), '1&amp;2' );
		$this->assertEqual( strmaxlen('12345678901234567890&amp;', 21, NULL, 'formvalue'),
			'12345678901234567890â€¦' );
		$this->assertEqual( strmaxlen('123456789012345&amp;', 21, NULL, 'formvalue'),
			'123456789012345&amp;amp;' );

		$this->assertEqual( strmaxlen('foo ', 3), 'foo' );
		$this->assertEqual( strmaxlen('foo ', 4), 'foo' );
		$this->assertEqual( strmaxlen('foo bar', 3), 'foâ€¦' );
		$this->assertEqual( strmaxlen('foo bar', 4), 'fooâ€¦' );
		$this->assertEqual( strmaxlen('foo bar', 5), 'fooâ€¦' );
		$this->assertEqual( strmaxlen('foo bar', 6), 'foo bâ€¦' );
	...
	function test_strmaxwords()
	{
		...
		$this->assertEqual( strmaxwords('  <img />foo  bar  ', 1), '  <img />foo  â€¦' );
	}
