...
$Form->end_fieldset();
	...
	{ // Display tags
		...
		$suggest_checkbox = ' <span title="'.format_to_output( T_('Check this to let b2evolution auto-suggest tags as you type, based on the tags existing on other posts.'), 'htmlattr' ).'">'
			...
			.'</span>';
		$Form->text_input( 'item_tags', $item_tags, 40, '<span class="nowrap">'.T_('Tags').$suggest_checkbox.'</span>', '', array( 'maxlength' => 255, 'style' => 'width: 99%;', 'required' => ( $edited_Item->get_type_setting( 'use_tags' ) == 'required' ) ) );
	}
	else
	{ // Hide tags
